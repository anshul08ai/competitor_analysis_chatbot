...
# # graph.add_edge("search_summarize_data","content_verifier_data")
# graph.add_edge("search_summarize_data","web_data_scrap")
# graph.add_edge("web_data_scrap","similar_db_data")
# graph.add_edge("similar_db_data",END)
# # graph.add_conditional_edges("content_verifier_data",needs_refinement_status,{True:"get_relevent_query",False: "analyze_generated_data"})

# graph.add_edge("analyze_generated_data","summarize_report")
# graph.add_edge("summarize_report",END)
compiled= graph.compile()

# compiled.invoke({'query':'Thanks for helping me?'})
