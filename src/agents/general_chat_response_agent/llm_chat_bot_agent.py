...
AZURE_CONFIG_PATH = Path(__file__).parent.parent.parent.parent  / 'config' / 'azure_config.yaml'
...
session_id = azure_config['SESSION_ID']
...
def llm_chat_bot(state):
    try:
        ...
        human_msg = HumanMessage(content=general_purpose_human_prompt.format(user_query=user_query, chat_history=format_chat_history))
